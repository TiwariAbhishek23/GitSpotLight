...
const Hero =() =>{
  ...
  if(loading){
   ...
   {
    return(
        <div className='w-1/2 mx-auto my-8 bg-gray rounded-4xl p-8'>
        {!userData && (
          <div className="container ">
            {/* <div className="heading text-4xl text-center ">Hi <span className='wave '>👋</span> <span className="gra">User</span></div> */}
            <div className="subheading text-center text-4xl m-4 p-4">Welcode <span className='wave '>👋</span> to <span className="grade">GitSpotLight</span></div>
            <div className="text-center text-4xl "> <div className="inline "><Type  /></div> </div>
          </div>
        )}
...
<div className="user">
            ...
            { name && <div className="name">👨‍💻 Name :  <div className="name-tab  inline">{name}</div></div>}
              { email && <div className="mail"> 📬 Email : {email}</div>}
             {/* { blog && <div className="blog">🔗 Blog Link : <Link  hreaf='/' target= "_blank" >Link</Link></div>} */}
              { twitterUserName && <div className="twitter">🐤 Twitter User Name : {twitterUserName}</div>}
              { company && <div className="company">💼 Company : {company}</div>}
              { startDate && <div className="date">🗓️ Joining Date : {startDate}</div>}
            </div>
          </div>
